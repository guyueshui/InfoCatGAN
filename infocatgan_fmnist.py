    ...
    def train(self):
        ...
        for epoch in range(1, 1 + self.config.num_epoch):
            ...
            for ux, _ in uloader:
                ...
                d_cost_fent = utils.Entropy( softmax(d_out_f, dim=1) )

                # use some of generated as real
                if epoch >= epoch_g:
                    g_noise, g_idx = self.generate_noise(bs)
                    g_fx = self.G(g_noise)
                ...
                d_cost = d_cost_ent - d_cost_ment - d_cost_fent + .5*d_cost_bind_g
                d_cost_list = [d_cost, d_cost_ent, d_cost_ment, d_cost_fent, d_cost_bind_g]
                ...
                g_optim.zero_grad()
                d_out_f_g = self.D(fx)
                g_cost_ent = utils.Entropy(softmax(d_out_f_g, dim=1))
                g_cost_ment = utils.MarginalEntropy(softmax(d_out_f_g, dim=1))
                g_cost_info = ce(d_out_f_g, idx)
                g_cost = g_cost_ent - g_cost_ment + alpha_info*g_cost_info
...
if __name__ == '__main__':
    ...
    """
    ARGS = GetConfig()
    assert ARGS.dbname == "FashionMNIST", "fmnist support only"
    #ARGS.nlabeled = 132
